  ...
  return (
    <div className="min-h-screen bg-background flex overflow-x-hidden">
      ...
      <aside className="hidden md:flex flex-col w-56 border-r border-border shrink-0">
        <div
          className="flex items-center gap-2.5 px-4 py-5 border-b border-border cursor-pointer"
          onClick={() => handleNav("/dashboard")}
        >
          <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center shrink-0">
            <Zap className="w-4 h-4 text-primary-foreground" />
          </div>
          <span className="font-bold text-lg tracking-tight truncate">Quizora</span>
        </div>
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-border z-40">
          <div className="flex items-center justify-around py-1.5 px-1">
            {navItems.map(item => {
              ...
              return (
                <div
                  key={item.href}
                  onClick={() => handleNav(item.href)}
                  className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-lg cursor-pointer min-w-0 flex-1 ${active ? "text-primary" : "text-muted-foreground"}`}
                >
            ...
            {isAdmin && (
              <div
                onClick={() => handleNav("/admin")}
                className={`flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-lg cursor-pointer min-w-0 flex-1 ${location.startsWith("/admin") ? "text-primary" : "text-muted-foreground"}`}
              >
                <Settings className="w-5 h-5 shrink-0" />
                <span className="text-[10px] leading-tight truncate max-w-full">Admin</span>
              </div>
            )}
          </div>
        </nav>
